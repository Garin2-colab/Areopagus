...
export async function POST() {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Modal simplify-keywords endpoint failed with status ${response.status} (URL: ${mutateUrl})`);
    }
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "Simplify keywords failed." },
      { status: 500 }
    );
  }
}
